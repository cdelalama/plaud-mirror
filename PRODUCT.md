...
recover authentication, curate recordings, and inspect failures without using
...
items, scheduler health, and downstream delivery state without the interface
claiming more than the runtime has observed.
...
1. State the ownership boundary. Local files, Plaud originals, scheduler state,
   and webhook delivery are separate facts.

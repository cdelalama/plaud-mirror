...
- `phase` reads `"Phase 3 - unattended operation"` when the scheduler is enabled, `"Phase 2 - first usable slice"` otherwise. The exact string is for human consumption; do not key logic off it.
...
- `v0.5.3` adds the durable webhook outbox (D-013): explicit FSM, exponential-backoff retry, the persisted queue replaces the immediate-only delivery path. New routes `GET /api/outbox` and `POST /api/outbox/:id/retry`. `v0.10.4` extends `health.outbox` with `delivering` (default 0 for older payloads) and corrects the retry ceiling to nine attempts separated by all eight documented waits. `SyncRunSummary.enqueued` counts queued payloads; `delivered` keeps its original semantic and structurally stays at 0.
...
- `v0.9.0` lands the reference-driven operator panel redesign. No route shape, webhook payload, or shared runtime schema changes; Main, Library, Backfill, Configuration, and Operations are frontend organization over the existing API surface.

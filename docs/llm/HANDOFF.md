...
- `dockit-sync` to LLM-DocKit **4.8.2** (upstream moved past the adopted 4.8.0 on 2026-05-17: DF-039 read-only-skip fix, file-existence checks, plus a new validator smoke-test script that arrives with the sync). Run `~/src/LLM-DocKit/scripts/dockit-sync.sh --project ~/src/plaud-mirror --dry-run` first; ship as its own patch release, separate from runtime work. Keep the local guardrails through the sync: upstream 4.8 orientation/template-residue checks plus `scripts/check-prose-drift.sh`, `scripts/check-unabsorbed-artifact.sh`, and `scripts/check-version-sync.sh` JSON package-version support.

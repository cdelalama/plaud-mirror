...
- **Operator action required to arm D-018 (one step left):** the Doppler side is ready — the operator bound this repo to the new secondary Doppler account (workplace "Startup Embassy", carlos@startupembassy.com) via `doppler login --scope ~/src/plaud-mirror` (the primary Xibstar account hit its 10-project free-tier limit; multi-account convention now documented in `~/src/home-infra/docs/CONVENTIONS.md` "Doppler Accounts"), and the Doppler project `plaud-mirror` exists there (created 2026-06-11 by the helper, closing the long-standing "create Doppler project" priority). **But the passphrase itself is NOT stored yet** — `plaud-mirror/dev` only contains the default `DOPPLER_*` keys (the helper run did not complete the secret-set step). Remaining: re-run `scripts/set-admin-passphrase.sh` from this directory, then `doppler run --project plaud-mirror --config dev -- docker compose up -d`, then verify `/api/session` reports `authRequired: true` and `/api/config` returns 401 without a cookie. Until then the deployed service runs open with the explicit `health.warnings` entry as the reminder.

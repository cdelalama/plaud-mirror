...
- `GET /api/health` returns `200` with `{ version: "0.5.1", auth.state: "healthy" }` against the operator's real Plaud account. The `scheduler` block is present in every response and **defaults to the disabled shape** when `PLAUD_MIRROR_SCHEDULER_INTERVAL_MS` is unset (the v0.5.0 default-on regression that caused this field to read `enabled: true` without operator opt-in is fixed in v0.5.1).

...
- Session Focus: Operator feedback on v0.4.12's backfill preview — "la interfaz, como es una tabla, se me sale completamente... en esa tabla, el dispositivo en lugar de meter el número deberías poner el nombre... y el curso de Scrolling Horizontal [implícitamente: no lo quiero]". Three issues: (1) the preview table overflowed the half-width card horizontally; (2) the Device column rendered the raw serial number instead of the operator's nickname; (3) horizontal scrolling was unacceptable.

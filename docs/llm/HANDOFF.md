...
- **Full health observability.** `lastErrors` ring buffer + extended outbox/sync history not yet on `/api/health` (D-014, scheduled for `v0.5.4`).
...
- The project is in **Phase 3** per [docs/ROADMAP.md](../ROADMAP.md). `v0.5.0` introduced the scheduler but regressed; `v0.5.1` fixed the regressions; `v0.5.2` added panel-driven configuration; `v0.5.3` adds the durable webhook outbox (D-013 done). The only remaining Phase 3 increment is full health observability (D-014 full → `v0.5.4`) — `lastErrors` ring buffer + extended outbox/sync history.
...
4. Ship `v0.5.4` (D-014 full, `lastErrors` ring buffer + extended outbox/sync history on `/api/health`).

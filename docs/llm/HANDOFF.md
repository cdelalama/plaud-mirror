...
- The project is in **Phase 3** per [docs/ROADMAP.md](../ROADMAP.md). `v0.5.0` introduced the scheduler but regressed; `v0.5.1` fixed the regressions; `v0.5.2` added panel-driven configuration; `v0.5.3` added the durable webhook outbox (D-013 done); `v0.5.4` was governance-only (Layer-1 doc-drift enforcement, D-016, no runtime change); `v0.5.5` (this release) ships D-014 full (`lastErrors` ring buffer + `recentSyncRuns`) plus governance (`prose-drift` WARN→FAIL, new `check_unabsorbed_artifact()` D-017). The Phase 3 runtime surface is feature-complete; remaining roadmap is multi-day soak verification, then Phase 4+ (resumable backfill, automatic re-login, NAS rollout, OSS polish).
...
- Home-infra control-plane exposure landed 2026-05-14 as commit `dec374f` on `cdelalama/home-infra` (catalog/services.yml + INVENTORY/PROJECTS/SERVICES synced to v0.5.5). Remaining local follow-up: confirm `docs/PROJECT_CONTEXT.md` references the live `https://plaud.lamanoriega.com/` route and sweep if it does not.
- Keep DocKit adoption honest after template sync: `scripts/dockit-validate-session.sh` must retain the upstream 4.8 orientation/template-residue checks plus local `scripts/check-prose-drift.sh`, `scripts/check-unabsorbed-artifact.sh`, and `scripts/check-version-sync.sh` JSON package-version support.

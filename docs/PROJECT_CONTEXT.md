...
## Current Status (2026-06-18, v0.9.4)

Plaud Mirror `v0.9.4` is the current **Phase 4 operator UX** patch. It does not change backend behavior. It keeps the `v0.9.0` reference-driven panel, the `v0.9.1` full-viewport shell, the `v0.9.2` Main cockpit sync fix, and the `v0.9.3` DocKit trace-protocol governance merge, then fixes the redesigned Library: Compact Play starts the native row audio, Full mode uses a wider desktop player column, and the recordings table owns its scroll region under the fixed Library header/toolbar/pagebar.
...
Operators upgrading from `0.4.x` should skip `v0.5.0` (scheduler default-on regression + missing service-layer anti-overlap) and go directly to `v0.9.4`.

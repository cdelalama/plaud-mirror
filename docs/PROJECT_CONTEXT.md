...
## Current Status (2026-06-19, v0.9.5)

Plaud Mirror `v0.9.5` is the current **Phase 4 operator UX** patch. It does not change backend behavior. It keeps the `v0.9.0` reference-driven panel, the `v0.9.1` full-viewport shell, the `v0.9.2` Main cockpit sync fix, the `v0.9.3` DocKit trace-protocol governance merge, and the `v0.9.4` Library playback/scroll fix, then makes the mobile shell usable: phone navigation now has a labeled `Vista` / `View` selector instead of icon-only rail buttons, the status strip collapses to one compact chip row, and Library dismiss/restore actions stay pinned to the top-right of mobile rows.
...
Operators upgrading from `0.4.x` should skip `v0.5.0` (scheduler default-on regression + missing service-layer anti-overlap) and go directly to `v0.9.5`.

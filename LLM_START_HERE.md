...
- Working on: **v0.7.6 (patch) — bookmarklet no longer fails silently**: operator tried the "Reconectar Plaud Mirror" bookmarklet on app.plaud.ai and saw no visible action. Fix: `buildBookmarklet` is now short (<2 KB), focuses on `pld_tokenstr` with a full storage scan fallback, and shows a Plaud Mirror alert for every outcome before returning to `/connect`. The panel now says that an alert must appear; if no alert appears, the marker did not install/execute. Tests 145. v0.7.5 (masked token + Docker deploy fix) below.

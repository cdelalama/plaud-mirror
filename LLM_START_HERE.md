...
- Working on: `v0.5.5` ships **D-014 full** (full health observability) — `/api/health` returns `lastErrors` (cross-subsystem ring buffer, in-memory, capped at 20) and `recentSyncRuns` (last 5 finished runs from SQLite). New `service.recordError(subsystem, message, context?)` is wired from `SchedulerManager.onTick`, `OutboxWorker.onDeliveryError`, and `service.runSync` catch path. Plus governance: `prose-drift` validator hardened `WARN`→`FAIL`; new `check_unabsorbed_artifact()` ninth validator check (D-017) with baseline file (3 entries: `check-prose-drift.sh` transient `df_id: DF-028`, `check-upstreams.sh` permanent, `external-context-triggers.md` permanent). Phase 3 runtime feature-complete. Tests 113 → 116 (105 backend + 11 web).

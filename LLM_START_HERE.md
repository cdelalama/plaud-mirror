...
- Working on: Closing the pending LLM-DocKit 4.8 sync and infra-exposure docs left uncommitted by the 2026-05-13 Codex session, shipped here as patch release **v0.5.6** (governance/sync; no runtime change). The `home-infra` control-plane side already shipped as `cdelalama/home-infra` commit `dec374f` (`docs: expose plaud-mirror in infra catalog`); a follow-up bump of `home-infra/docs/PROJECTS.md` from v0.5.5 to v0.5.6 is owed alongside this release. The v0.5.6 bump itself was forced by the pre-commit hook (scripts/* and .claude/settings.json count as versioned governance surface, same pattern as v0.5.4 and v0.5.5). Upstream LLM-DocKit 4.8 sync output is intact (SessionStart hook, `scripts/dockit-bootstrap-context.sh`, extended validator, version-sync manifest) and local guardrails preserved (`json-version`, D-016, D-017).

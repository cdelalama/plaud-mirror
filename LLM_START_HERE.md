...
- Working on: operator feedback on v0.4.11's backfill form — "en controls, no debería haber una parte abajo en la que si selecciono un dispositivo u otro me dice qué audios están por bajar. Yo aquí no veo qué audios hay. El campo Scene, escena, no es útil porque no sé qué puedo meter ahí." Two issues: (1) no preview of what a backfill would touch before clicking, (2) Scene field was opaque and unusable.
- Status: `v0.4.12` ships a backfill dry-run preview + removes Scene from the UI. New `GET /api/backfill/candidates` endpoint reuses the sync pipeline's first half (`client.listEverything` + `applyLocalFilters`) and annotates each matching recording with its current local state (`missing`/`mirrored`/`dismissed`) without downloading anything. New shared schemas: `BackfillCandidateStateSchema`, `BackfillCandidateSchema`, `BackfillPreviewFiltersSchema`, `BackfillPreviewResponseSchema`. New service method `previewBackfillCandidates` (reuses `toStartTimestamp`/`toEndTimestamp`/`applyLocalFilters` from the real sync path so preview cannot drift from reality). New server route with query-string parsing helpers `parseOptionalInt` / `parsePreviewLimit`. Web panel: `<BackfillPreview>` component inside the Historical backfill card, debounced 500 ms on filter changes, renders a table with #N, Title, Date, Duration, Device, and a colored state badge; header shows "X match — Y would be downloaded (of Z total in Plaud)"; truncates to 200 rows with a "Showing first 200 of M" footer. Scene `<input>` removed from the backfill form (BackfillDraft no longer has a `scene` field; handleRunBackfill no longer sends it; the backend schema still accepts scene for programmatic callers so no breaking change). Two new tests: service `previewBackfillCandidates` (state annotations + serial filter + previewLimit cap); server `GET /api/backfill/candidates` (no-filter + serial-filter). 53/53 pass. Docs: CHANGELOG 0.4.12, ARCHITECTURE new "Backfill preview" sub-section + updated header, PROJECT_CONTEXT current-status prose, ROADMAP current target, API_CONTRACT new endpoint row + full response-shape section. Next: rebuild image, verify VERSION, commit + push.

...
- Working on: **v0.7.1 (patch) — UX fixes to the v0.7.0 re-auth flow**: synchronous tab-open so mobile popup blockers don't reject "Reconectar Plaud" (captureId minted in parallel), plus a "Copiar marcador" button for mobile bookmarklet install. No security change; tests still 141. Operator still needs to run the reconnect to fix the Plaud token (degraded since 2026-05-13). Feature release below.
